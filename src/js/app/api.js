...
async function loadSimpleToolsConfig() {
    ...
    return { read_file: true, write_file: true, edit_file: true, shell_run: true, output_limit_kb: 99 };
}
...
async function getChatHistory(chatId = null) {
    try {
        const url = chatId
            ? `${API_BASE}/api/chat/${chatId}/history`
            : `${API_BASE}/api/chat/${currentChatId}/history`;
