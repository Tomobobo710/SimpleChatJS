...
function formatMessage(text) {
    ...
    return text
        // Headings: # through ###### at start of a line. Runs first so the #
        // markers don't get swallowed by later replacements. The trailing
        // newline is consumed (not just lookahead) so it doesn't become a <br>
        // after the block heading — that would add an unwanted blank line.
        // A lookbehind anchors the start so consecutive headings still match.
        ...
        .replace(/((?:^|\n|(?<=<\/h[1-6]>))\|[^\n]+\|\n\|[ \t\-:|]+\|(?:\n\|[^\n]+\|)*\n?)/g, function(match) {
            ...
            if (bodyLines.length) {
                html += '<tbody>' + bodyLines.map(function(line) {
                    return buildRow(parseCells(line), 'td');
                }).join('') + '</tbody>';
            }
            return html + '</table>';
        })
...
// Collapse blank lines out of RESPONSE chat content. Model responses often arrive with
// double newlines between paragraphs (plus stray leading/trailing ones); for responses we
// don't want any visible blank line in the chat block. Runs of 2+ newlines (with optional
// intervening whitespace) collapse to a single newline; leading/trailing blanks are
// trimmed. Single line breaks are preserved. Fenced code blocks are kept verbatim (blank
// lines there are meaningful) — response code is normally split into its own block before
// reaching here, but this is a cheap safety net. Request turns deliberately skip this.
function collapseResponseBlankLines(text) {
    ...
    s = s
        .replace(/[ \t]*\r?\n(?:[ \t]*\r?\n)+/g, '\n')  // runs of blank lines → one newline
        .replace(/^[ \t]*(?:\r?\n)+/, '')               // leading blank lines
        .replace(/(?:\r?\n)+[ \t]*$/, '');              // trailing blank lines
    return s.replace(/\uE000F(\d+)\uE001/g, (m, i) => fences[+i]);
}
