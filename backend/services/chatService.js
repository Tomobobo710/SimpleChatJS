...
// Turn-based debug data keyed on turn_id for sibling safety.
async function saveTurnDebugData(chatId, turnId, debugData) {
    const { db } = require("../config/database");

    try {
        if (!turnId) {
            // Skip if no turn_id yet (aborted stream).
        ...
        // Update existing message row. Caller ensures the row exists.
...
// ('root' or a turn_id). Survives reloads and restarts.
...
async function saveCompleteMessageToDatabase(
    chatId,
    messageData,
    turnNumber = null,
    errorState = null,
...
async function processChatRequest(req, res) {
    const { db } = require("../config/database");
    try {
   // req.body.message is intentionally not read — the backend builds
        // the LLM messages array from DB history only.
        ...
        if (lineage_anchor_turn_id) {
            ...
            if (anchorMsg) {
                ...
                log(
                    `[CHAT] Retry detected: filtering history to selected turn lineage (maxTurnId=${historyMaxTurnId})`
                );
            }
        } else if (parent_turn_id) {
            // Fallback: find first assistant with this parent_turn_id
            const anchorAssistant = db
                .prepare(
...
module.exports = {
    ...
    getTurnInfo,
    incrementTurnNumber,
    // Turn-based debug data functions (keyed on turn_id).
    saveTurnDebugData,
    getTurnDebugData,
